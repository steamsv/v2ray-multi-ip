...
for ip in $ip_list; do
    ...
    newdata=$(jq --arg ip "$ip" --arg tag "$tag" --arg uuid "$uuid" --arg user "$user" --arg i "$i" '.inbounds += [{"listen":"0.0.0.0","email":$user,"port":$i,"protocol":"shadowsocks","settings":{"method":"aes-256-gcm","password":"hvfdghvufgv","network":"tcp,udp"}}] | .outboundDetour += [{"sendThrough":$ip,"protocol":"freedom","tag":$tag}] | .routing.rules += [{"type":"field","user":[$user],"outboundTag":$tag}]' <<< "$data") || exit 1 # 检查JSON语法是否正确
