...
for ip in $ip_list; do
    ...
    newdata=$(jq --arg ip "$ip" --arg tag "$tag" --arg uuid "$uuid" --arg user "$user" '.inbound.settings.clients += [{"listen":"0.0.0.0","port":$i,"protocol":"shadowsocks","settings":{"clients":[{"email":$user,"method":"aes-256-gcm","password":"hvfdghvufgv","level":0,"network":"tcp,udp"}]}}] | .outboundDetour += [{"sendThrough":$ip,"protocol":"freedom","tag":$tag}] | .routing.rules += [{"type":"field","user":[$user],"outboundTag":$tag}]' <<< "$data") || exit 1 # 检查JSON语法是否正确
    echo "$newdata" > /usr/local/etc/v2ray/config.json || exit 1 # 检查写入是否成功
    echo "ss://YWVzLTI1Ni1nY206aHZmZGdodnVmZ3Y=@${ip}:8090#ss"
done
